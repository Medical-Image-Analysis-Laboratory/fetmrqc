...
def compute_segmentations(bids_df, out_path, model, ckpt_path):
    ...
    if model == "nnUNet":
        ...
        Parallel(n_jobs=4)(delayed(mask_im)(im,mask) for im,mask in zip(bids_df["im"], bids_df["mask"]))
        print(" done.")
        ...
        for file in list(out_path.glob("*_T2w.nii.gz")):
            ...
            df = pd.concat([df,pd.DataFrame({"sub":str(sub),"ses":ses,"run":run,"seg":seg_out,"seg_proba":seg_proba},index=[0])],ignore_index=True)
    ...
    new_columns = [col for col in bids_df.columns if col != 'seg' and col != 'seg_proba']
...
def load_and_run_segmentation(bids_csv, out_path, model, ckpt_path):
    ...
    df["sub"] = df["sub"].apply(lambda x: x if isinstance(x, str) else f"{x:03d}")
