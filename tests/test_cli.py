...
def compare_dataframes(df1, df2, numeric_tolerance=1e-6):
    ...
    num_compare = (
        np.isclose(
            df1[numeric_cols], df2[numeric_cols], rtol=numeric_tolerance
        )
        .all()
        .all()
    )
...
def test_run_pipeline():
    ...
    for root, dirs, files in os.walk(out_dir):
        for file in files:
            list_files.append(
                os.path.join(root, file).replace(str(out_dir) + "/", "")
            )
    list_files.sort()
    ...
    assert list_files == [line.replace("\n", "") for line in txt]
...
def mock_iqms(n_rows=50):
    ...
    while df.shape[0] < n_rows:
        ...
        for run in range(n_runs):
            ...
            row = [
                np.random.bytes(5).hex(),
                sub_ses_str,
                run + 1,
                np.random.randint(1, 5),
            ] + np.random.rand(len(cols) - 4).tolist()
...
def test_run_iqms():
    ...
    dataset = {
        "dataset_path": str(FILE_DIR / "output/test_run_iqms.csv"),
        "first_iqm": "centroid",
    }
    experiment = {
        "type": "regression",
